...
Start-Process -Wait -NoNewWindow -FilePath "py" -Args "`"$IDF_PATH/tools/idf_tools.py`" check-python-dependencies"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE } # if error

$uninstall = py "$IDF_PATH/tools/idf_tools.py" uninstall --dry-run
...
if (![string]::IsNullOrEmpty($uninstall)){
    ...
    Write-Output "For free up even more space, remove installation packages of those tools. Use option 'py.exe $IDF_PATH\tools\idf_tools.py uninstall --remove-archives'."
    Write-Output ""
}
